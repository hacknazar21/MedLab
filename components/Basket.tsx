...
export function Basket(props: Props) {
  ...
  const closeClickHandler: React.MouseEventHandler<HTMLButtonElement> = (
    event
  ) => {
    setProducts((prev) => {
      // @ts-ignore
  ...
  return (
    <div className="basket basket-window">
      ...
      <div className="basket__items">
        {products &&
          products.map((product: IAnalys, index) => {
            return (
      ...
      <div className="basket__sum">Итого: {totalPrice} тг</div>
      <Link href="/checkout">
        <a className="basket__checkout">Оплатить анализ</a>
      </Link>
    </div>
  );
}
